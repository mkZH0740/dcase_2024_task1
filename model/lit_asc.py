...
class LitAcousticSceneClassificationSystem(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        x = (
            self.spec_extractor(x).unsqueeze(1)
        ...
        if mix_up is not None:
            ...
            train_loss = mix_up.lam * F.cross_entropy(y_hat, y[0]) + (
                1 - mix_up.lam
            ) * F.cross_entropy(y_hat, y[1])
            corrects = mix_up.lam * torch.sum(pred == y[0]) + (
                1 - mix_up.lam
            ) * torch.sum(pred == y[1])
    ...
    def on_test_epoch_end(self):
        ...
        model_profile = torchinfo.summary(
            self.backbone, input_size=self._test_input_size
        )
...
class LitAscWithKnowledgeDistillation(LitAcousticSceneClassificationSystem):
    ...
    def __init__(
        self, temperature: float, kd_lambda: float, logits_index: int = -1, **kwargs
    ):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Calculate the loss and accuracy
        if mix_up is not None:
            label_loss = mix_up.lam * F.cross_entropy(y_hat, y[0]) + (
                1 - mix_up.lam
            ) * F.cross_entropy(y_hat, y[1])
            kd_loss = mix_up.lam * self.kl_div_loss(y_hat_soft, y_soft[0]) + (
                1 - mix_up.lam
            ) * self.kl_div_loss(y_hat_soft, y_soft[1])
        else:
        ...
        self.log_dict(
            {"loss": loss, "label_loss": label_loss, "kd_loss": kd_loss},
            on_step=False,
            on_epoch=True,
            prog_bar=True,
            logger=True,
...
class LitAscWithWarmupLinearDownScheduler(LitAcousticSceneClassificationSystem):
    ...
    def configure_optimizers(self):
        ...
        schedule_lambda = exp_warmup_linear_down(
            self.warmup_len, self.down_len, self.warmup_len, self.min_lr
        )
...
class LitAscWithTwoSchedulers(LitAcousticSceneClassificationSystem):
    ...
    def __init__(
        self,
        optimizer: OptimizerCallable,
        scheduler1: LRSchedulerCallable,
        scheduler2: LRSchedulerCallable,
        milestones,
        **kwargs,
    ):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.SequentialLR(
            optimizer, [scheduler1, scheduler2], self.milestones
        )
        return {"optimizer": optimizer, "lr_scheduler": scheduler}
...
class LitAscWithThreeSchedulers(LitAcousticSceneClassificationSystem):
    ...
    def __init__(
        ...
        scheduler1: LRSchedulerCallable,
        scheduler2: LRSchedulerCallable,
        scheduler3: LRSchedulerCallable,
        milestones,
        **kwargs,
    ):
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.SequentialLR(
            optimizer, [scheduler1, scheduler2, scheduler3], self.milestones
        )
        return {"optimizer": optimizer, "lr_scheduler": scheduler}
