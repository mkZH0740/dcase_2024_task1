...
class AdaResNorm(nn.Module):
    def __init__(self, channels: int, grad=True, eps=1e-5):
        ...
        self._eps = torch.full((1, channels, 1, 1), eps)
        self._rho = (
            nn.Parameter(torch.full((1, channels, 1, 1), 0.5))
            if grad
            else torch.full((1, channels, 1, 1), 0.5)
        )
...
class TimeFreqSepConvolutions(nn.Module):
    ...
    def __init__(
        self, in_channels: int, out_channels: int, kernel_size: int, dropout_rate: float
    ):
